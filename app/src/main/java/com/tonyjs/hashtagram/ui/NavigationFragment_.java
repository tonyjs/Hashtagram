...
import com.tonyjs.hashtagram.io.model.NavigationItem_;
 ...
 */
public class NavigationFragment_ extends BaseFragment
                implements BasicRecyclerView.OnItemClickListener{

    public interface NavigationCallback {
        public void onItemSelected(NavigationItem_ item);
    }

    public interface LifecycleCallback {
        public void onFragmentCreated(NavigationFragment_ fragment);
    }
    ...
    public void updateUi() {
        ...
        if (!TextUtils.isEmpty(profileImageUrl)) {
            ImageLoader.load(
                    mActivity, mIvUser, profileImageUrl, ImageLoader.TransformationType.CIRCLE);
        } else {
    ...
    public void updateNavigationItems() {
        ...
        ArrayList<NavigationItem_> pageItems = new ArrayList<>();
        int i = 0;
        for (String hashTag : items) {
            pageItems.add(new NavigationItem_(i, hashTag));
            i++;
        }
    ...
    private void revertItems() {
        ...
        for (int i = 0; i < max; i++) {
            NavigationItem_ item = mAdapter.getItem(i);
    ...
    private void callback(int position) {
        ...
        NavigationItem_ item = mAdapter.getItem(position);
    ...
    private class NavigationAdapter extends BasicRecyclerAdapter<NavigationItem_> {
        ...
        class NavigationHolder extends BasicViewHolder<NavigationItem_> {
            ...
            @Override
            public void onBindView(NavigationItem_ item) {
